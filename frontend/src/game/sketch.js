...
function sketch(p5) {
  ...
  async function classifyVideo(htmlVideoElement) {
    ...
    console.log(
      "SCORE, CARD",
      score[indexOfMaxValue],
      KEY_TO_CARD[cards[indexOfMaxValue]]
    );
  ...
  p5.setup = () => {
    ...
    navigator.mediaDevices
      ...
      .then((stream) => {
        ...
        // First do Automatic Way, so that if card is not detected, we can still play the game

        // socket.emit("getRoom", { room }, (roomObject) => {
        //   if (roomObject) {
        //     let index = roomObject.players.indexOf(username);
        //     let card_value = roomObject.player_card_value[index];
        //     let card_suit = roomObject.player_card_suit[index];
        //     card = new Card(
        //       0,
        //       0,
        //       SUIT_ICONS[card_suit],
        //       VALUES[parseInt(card_value)],
        //       true,
        //       p5
        //     );
        //     user = new User(0, 0, username, card, video, p5);
        ...
        // Update the tensor flow model
        // Then do Tensorflow Way, so that if card is detected, we can update the card
